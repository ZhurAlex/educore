...
**Data access: via API, built.** `GET /api/test_attempts` (`app/controllers/api/`, `app/serializers/api/`), token-authenticated, filterable by test/student/school_class/subject. Full contract — request/response shape, auth header, known gaps (no pagination, no versioning yet): `docs/API_CONTRACT.md`. **If you're touching that API, also read `../educore-analytics/CLAUDE.md`** — it documents what the consumer actually expects to read; not auto-imported here since most work in this repo has nothing to do with the analytics side.
